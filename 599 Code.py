...
ail_data1 = pd.read_csv(
    'C:/Users/Colton/Documents/Econ 611 MSA data/AIL and Pool Price (2010-2020).csv')
ail_data2 = pd.read_csv(
    'C:/Users/Colton/Documents/Econ 611 MSA data/AIL and Pool Price (2017-2020).csv')
temp_data = pd.read_csv(
    'C:/Users/Colton/Documents/Econ 611 MSA data/Wide Format_Weighted Temp 2010-2021.csv')
oilfutures_data = pd.read_csv(
    r'C:\Users\Colton\Documents\Econ 611 MSA data\EIA NYMEX Futures (Crude Oil).csv', header=2)
oilprices_data = pd.read_csv(
    r'C:\Users\Colton\Documents\Econ 611 MSA data\EIA Oil Prices.csv', header=2)

# Cleaning the Data
oilfutures_data = oilfutures_data.rename(columns={"Cushing, OK Crude Oil Future Contract 1 (Dollars per Barrel)": "future 1", "Cushing, OK Crude Oil Future Contract 2 (Dollars per Barrel)": "future 2",
                                         "Cushing, OK Crude Oil Future Contract 3 (Dollars per Barrel)": "future 3", "Cushing, OK Crude Oil Future Contract 4 (Dollars per Barrel)": "future 4"})
...
oilprices_data = oilprices_data.rename(
    columns={"Cushing, OK WTI Spot Price FOB (Dollars per Barrel)": "WTI spot"})
...
temp_data = temp_data.rename(
    columns={"Avg temp": "Avg_temp", "Weighted Avg Temp": "Weighted_Avg_Temp"})
temp_datacln = temp_data[['BEGIN_DATE_GMT', 'Avg_temp', 'Weighted_Avg_Temp']]
temp_datacln['Avg_temp'] = temp_datacln['Avg_temp'].astype(float)
temp_datacln['Weighted_Avg_Temp'] = temp_datacln['Weighted_Avg_Temp'].astype(
    float)
...
merge_data["POOL_PRICE"] = merged_data["POOL_PRICE"].replace(
...
merged_data["holiday"] = [
    1 if i.date() in canada_holidays else 0 for i in data["BEGIN_DATE_GMT"]]
...
normal_data['Weighted_Avg_Temp'] = normal_data['Weighted_Avg_Temp'].astype(
    float)
